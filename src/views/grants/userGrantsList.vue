  ...
  <div class="app-container">
    ...
    <div v-if="grantsActive === 'schema'">
      ...
      >
        ...
        <el-table-column label="用户名称" width="150" align="left">
          <template slot-scope="scope">{{ scope.row.objectName }}</template>
        </el-table-column>
        <el-table-column label="所属角色" width="150" align="left">
          <template slot-scope="scope">
            <span>{{ scope.row.assigendusers.replace(/,/g, "\n") }}</span>
          </template>
        </el-table-column>
      ...
      <div v-if="schemaGrantsFormState" class="wmassageMask" @click="closeSchemaGrantsDialog">
        ...
        >
          ...
          >
            ...
            <el-form-item label="用户名称" prop="objectName">
              <el-select v-if="schemaGrantsForm.id === ''" v-model="schemaGrantsForm.objectName"
                         placeholder="请选择用户名称" style="width: 100%"
              >
                <el-option v-for="(item, n) in userList" :key="n" :label="
                             item.username +
                               (item.userstate === '0' ? '(未绑定实例)' : '(已启用)')
                           " :value="item.username" :style="item.userstate === '0' ? 'color:red' : ''"
            ...
            <el-form-item label="所属角色">
              <span>{{ schemaGrantsForm.assigendusers }}</span>
            </el-form-item>
            <el-form-item label="角色外权限">
                ...
                <el-table-column label="权限配置">
                  <template slot-scope="scope">
                    ...
                    <el-checkbox-group v-model="scope.row.checkGrantsList" @change="handleChangeChecked(scope.row)">
                      <el-checkbox v-for="(grant, optionIndex) in grantsList" :key="optionIndex" :label="grant">{{ grant
                      }}</el-checkbox>
                    </el-checkbox-group>
                  </template>
                </el-table-column>
            ...
            <div style="text-align: center">
              <el-button v-if="schemaGrantsForm.id === ''" type="primary" @click="createSchemaGrants">创建
              </el-button>
    ...
    <div v-if="grantsActive === 'external'">
      ...
      >
        ...
        <el-table-column label="用户名称" width="250" align="left">
          <template slot-scope="scope">{{ scope.row.objectName }}</template>
        </el-table-column>
        <el-table-column label="所属角色" width="300" align="left">
          <template slot-scope="scope">
            <span>{{ scope.row.assigendusers.replace(/,/g, "\n") }}</span>
          </template>
        </el-table-column>
      ...
      <div v-if="externalGrantsFormState" class="wmassageMask" @click="closeExternalGrantsDialog">
        ...
        >
          ...
          <el-form ref="externalGrantsForm" :model="externalGrantsForm" :rules="externalGrantsRules" label-width="120px"
                   label-position="right"
          >
            <el-input v-model="externalGrantsForm.id" type="hidden" />
            <el-form-item label="用户名称" prop="objectName">
              <el-select v-if="externalGrantsForm.id === ''" v-model="externalGrantsForm.objectName" placeholder="请选择用户名称" style="width: 100%">
                <el-option v-for="(item, n) in userList" :key="n" :label=" item.username + (item.userstate === '0' ? '(未绑定实例)' : '(已启用)')" 
                           :value="item.username" :style="item.userstate === '0' ? 'color:red' : ''"
                           @click.native="selectedExternalUserName(item)"
                />
              </el-select>
              <span v-else>{{ externalGrantsForm.objectName }}</span>
            </el-form-item>
            <el-form-item label="所属角色">
              <span>{{ externalGrantsForm.assigendusers }}</span>
            </el-form-item>
...
export default {
  data() {
    return {
      ...
      schemaGrantsRules: {
        objectName: [
          { required: true, trigger: "change", message: "请选择用户名称" },
        ],
      },
      externalGrantsList: [],
  ...
  methods: {
    ...
    getSchemaGrantsList() {
      ...
      let params = {
        objectName: this.searchObjectName,
        objectType: "USER",
      };
      let that = this;
    ...
    createSchemaGrants() {
      this.$refs.schemaGrantsForm.validate((valid) => {
        if (valid) {
          ...
          if (this.schemaGrantsForm.userState === "0") {
            Message({
              message:
                "当前用户未绑定实例需要先虚拟订购相关实例才能分配schema权限！！！",
              type: "error",
              duration: 5 * 1000,
            });
            return false;
    ...
    deleteSchemaGrants(row) {
      // to re-login
      MessageBox.confirm("确定要删除当前用户SCHEMA权限么？", "警告提示", {
    ...
    selectedUserName(userInfo) {
      this.schemaList = [];
      this.schemaGrantsForm.userState = userInfo.userstate;
      if (userInfo.userstate === "0") {
        ...
        getRoleList(params).then((response) => {
          ...
          that.roleList.forEach((res) => {
            let assigendusers = res.assigendusers.split(",");
            assigendusers.forEach((element) => {
              if (element === userInfo.username) {
                checkedRoles.push(res.rolename);
              }
            });
          });
          this.schemaGrantsForm.assigendusers = checkedRoles.join(",");
        });

        let params1 = {
          schemaname: "",
        };
        getSchemaList(params1).then((response) => {
    ...
    clearSchemaGrantsDialog() {
      ...
      this.schemaGrantsForm.objectName = "";
      this.schemaGrantsForm.objectType = "USER";
      this.schemaGrantsForm.userState = "0";
      this.schemaGrantsForm.assigendusers = "";
    ...
    deleteExternalGrants(row) {
      // to re-login
      MessageBox.confirm("确定要删除当前用户外部函数权限么？", "警告提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        ...
        deleteExternalGrants(params)
          ...
          .catch(() => {
            console.log("删除用户外部函数失败!!");
            this.listLoading = false;
          });
      });
    },
    /** 用户称选中事件 */
    selectedExternalUserName(userInfo) {
      if (userInfo.userstate === "0") {
        Message({
          message:
            "当前用户未绑定实例需要先虚拟订购相关实例才能分配外部函数权限！！！",
          type: "error",
          duration: 5 * 1000,
        });
        ...
        getRoleList(params).then((response) => {
          ...
          that.externalGrantsForm.assigendusers = checkedRoles.join(",");
        });
    ...
    openEditExternalGrantsDialog(row) {
      ...
      getVsProceduresList({}).then((response) => {
        that.externalList = response.data;
        that.$nextTick(() => {
          let privilegesDescription =
            that.externalGrantsForm.privilegesDescription.split(";");
    ...
    clearExternalGrantsDialog() {
      ...
      (this.externalGrantsForm.objectType = "USER"),
        (this.externalGrantsForm.assigendusers = "");
      this.externalGrantsForm.privilegesDescription = "";
      this.externalList = [];
      this.$nextTick(() => {
